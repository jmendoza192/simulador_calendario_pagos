...
# --- 4. DEFINICIÓN DE PESTAÑAS (CRÍTICO PARA EVITAR NAMEERROR) ---
...
with tab2:
    ...
    st.markdown(f"""
        <div style="background-color: {color_rec}; padding: 20px; border-radius: 15px; text-align: center; margin-bottom: 25px; border: 2px solid #ffffff33;">
            ...
            <p style="color: #e0e0e0; font-size: 1.1rem; margin-top: 10px;">
                Basado en el análisis de TCEA y seguros, el <b>{mejor_banco}</b> es la opción más rentable.
            </p>
        </div>
    """, unsafe_allow_html=True)
    ...
    fig_comp.update_layout(barmode='group', paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', font_color="white", legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    fig_comp.update_yaxes(tickprefix="S/ ", tickformat=",.0f")
    st.plotly_chart(fig_comp, use_container_width=True)

    # --- TABLA DE RESUMEN NUMÉRICO ---
    st.subheader("📋 Resumen Numérico")
    tabla_v = comp_df.copy()
    tabla_v["Banco A"] = tabla_v["Banco A"].map("S/ {:,.0f}".format)
    tabla_v["Banco B"] = tabla_v["Banco B"].map("S/ {:,.0f}".format)
    st.table(tabla_v)
